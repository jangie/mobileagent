module.exports = function(ua) {
	...
	if (/like Mac OS X/.test(ua)) {
		...
		if(typeof v.length === 'number' && v.length >= 3) $.iOS = [2].replace(/_/g, '.');
		else $.iOS = true;
	...
	if (/Android/.test(ua)) {
		...
		if(typeof v.length === 'number' && v.length >= 2) $.Android = v[1];
		else $.Android = true;
	}
	...
	if (/webOS\//.test(ua)) {
		...
		if(typeof v.length === 'number' && v.length >= 2) $.webOS = v[1];
		else $.webOS = true;
	}
	...
	if (/(Intel|PPC) Mac OS X/.test(ua)) {
		...
		if(typeof v.length === 'number' && v.length >= 3) $.Mac = v[2].replace(/_/g, '.');
		else $.Mac = true;
	}
	...
	if (/Windows NT/.test(ua)) {
		...
		if(typeof v.length === 'number' && v.length >= 2) $.Windows = v[1];
		else $.Windows = true;
	}
